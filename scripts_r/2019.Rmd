---
title: "Socio hackaton. Año 2019"
author: "micaeladehernadnez"
date: "2024-10-10"
...
```{r base de eph}
# 2. Descargar EPH del 4to trismestre 2019.
# Una con sólo población PEA.
EPH <- get_microdata(year = 2019, period = 4, type = "individual")
```
...
# 3. País por región
Subocupados.por.región.2019 <- EPH %>%
  group_by(REGION) %>%
  summarise(
    Poblacion = sum(PONDERA),
    Ocupados = sum(PONDERA[ESTADO == 1]),
...
Proporcion.ingresos.laborales.por.sexo.2019 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
...
Proporcion.ingresos.laborales.por.sexo.informalidad.2019 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
  filter(Proporcion.ingresos.laborales > 0) %>%
...
Proporcion.ingresos.laborales.por.region.2019 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
  filter(Proporcion.ingresos.laborales > 0) %>%
  group_by(REGION) %>%
...
# General
Tablas_unidas_2019 <- data.frame(
  Año = 2019,
  Actividad = Actividad.2019[1, "Tasa.Actividad"],
  Empleo = Empleo.2019[1, "Tasa.Empleo"],
  Desempleo = Desempleo.2019[1, "Tasa.Desempleo"],
  Informalidad = Informalidad[1, "Tasa.Informalidad"],
  Subocupados = SubocupadoS.2019[1, "Tasa.Subocupados"]
)


write.csv(Tablas_unidas_2019, "data/eph/2019/General.csv")

# Por sexo
Tablas_unidas_sexo_2019 <- data.frame(
  Año = 2019,
  Sexo = Actividad.sexo.2019["Sexo"],
  Actividad = Actividad.sexo.2019["Tasa.Actividad"],
  Empleo = Empleo.sexo.2019["Tasa.Empleo"],
  Desempleo = Desempleo.sexo.2019["Tasa.Desempleo"],
  Informalidad = Informalidad.por.sexo["Tasa.Informalidad"],
  Subocupados = Subocupados.por.sexo.2019["Tasa.Subocupados"],
  Proporción_de_ingresos_laborales_sobre_totales = Proporcion.ingresos.laborales.por.sexo.2019["PromedioProporcion"]
)


write.csv(Tablas_unidas_sexo_2019, "data/eph/2019/Sexo.csv")


# Por región
Tablas_unidas_region_2019 <- data.frame(
  Año = 2019,
  Region = Actividad.Región.2019["REGION"],
  Actividad = Actividad.Región.2019["Tasa.Actividad"],
  Empleo = Empleo.región.2019["Tasa.Empleo"],
  Desempleo = Desempleo.región.2019["Tasa.Desempleo"],
  Informalidad = Informalidad.por.región["Tasa.Informalidad"],
  Subocupados = Subocupados.por.región.2019["Tasa.Subocupados"],
  Proporción_de_ingresos_laborales_sobre_totales = Proporcion.ingresos.laborales.por.region.2019["PromedioProporcion"]
)

write.csv(Tablas_unidas_region_2019, "data/eph/2019/Region.csv")
```