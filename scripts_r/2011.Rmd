...
```{r base de eph}
#2. Descargar EPH del 4to trismestre 2011.
#Una con sólo población PEA.
EPH<-get_microdata(year = 2011, period= 4, type = "individual")
```
...
#3. País por región
Subocupados.por.región.2011 <- EPH %>%
  group_by(REGION) %>%
  summarise(Poblacion = sum(PONDERA),
            Ocupados = sum(PONDERA[ESTADO == 1]),
            Desocupados = sum(PONDERA[ESTADO == 2]),
...
Proporcion.ingresos.laborales.por.sexo.2011 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
...
Proporcion.ingresos.laborales.por.sexo.informalidad.2011 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
  filter(Proporcion.ingresos.laborales > 0) %>%
...
Proporcion.ingresos.laborales.por.region.2011 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
  filter(Proporcion.ingresos.laborales > 0) %>%
  group_by(REGION) %>%
...
#General
Tablas_unidas_2011 <- data.frame(
  Año = 2011,
  Actividad = Actividad.2011[1,"Tasa.Actividad"],
  Empleo = Empleo.2011[1,"Tasa.Empleo"],
  Desempleo = Desempleo.2011[1,"Tasa.Desempleo"],
  Informalidad = Informalidad[1,"Tasa.Informalidad"],
  Subocupados = Subocupados.2011[1,"Tasa.Subocupados"]
  )


write.csv(Tablas_unidas_2011, "data/eph/2011/General.csv")

#Por sexo
Tablas_unidas_sexo_2011 <- data.frame(
  Año = 2011,
  Sexo = Actividad.sexo.2011["Sexo"],
  Actividad = Actividad.sexo.2011["Tasa.Actividad"],
  Empleo = Empleo.sexo.2011["Tasa.Empleo"],
  Desempleo = Desempleo.sexo.2011["Tasa.Desempleo"],
  Informalidad = Informalidad.por.sexo["Tasa.Informalidad"],
  Subocupados = Subocupados.por.sexo.2011["Tasa.Subocupados"],
  Proporción_de_ingresos_laborales_sobre_totales =  Proporcion.ingresos.laborales.por.sexo.2011["PromedioProporcion"]
  )


write.csv(Tablas_unidas_sexo_2011, "data/eph/2011/Sexo.csv")


#Por región
Tablas_unidas_region_2011 <- data.frame(
  Año = 2011,
  Region = Actividad.Región.2011["REGION"],
  Actividad = Actividad.Región.2011["Tasa.Actividad"],
  Empleo = Empleo.región.2011["Tasa.Empleo"],
  Desempleo = Desempleo.región.2011["Tasa.Desempleo"],
  Informalidad = Informalidad.por.región["Tasa.Informalidad"],
  Subocupados = Subocupados.por.región.2011["Tasa.Subocupados"],
  Proporción_de_ingresos_laborales_sobre_totales =  Proporcion.ingresos.laborales.por.region.2011["PromedioProporcion"]
  )

write.csv(Tablas_unidas_region_2011, "data/eph/2011/Region.csv")
