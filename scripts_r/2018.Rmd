...
```{r base de eph}
# 2. Descargar EPH del 4to trismestre 2018.
# Una con sólo población PEA.
EPH <- get_microdata(year = 2018, period = 4, type = "individual")
```
...
# 3. País por región
Subocupados.por.región.2018 <- EPH %>%
  group_by(REGION) %>%
  summarise(
    Poblacion = sum(PONDERA),
    Ocupados = sum(PONDERA[ESTADO == 1]),
...
Proporcion.ingresos.laborales.por.sexo.2018 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
...
Proporcion.ingresos.laborales.por.sexo.informalidad.2018 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
  filter(Proporcion.ingresos.laborales > 0) %>%
...
Proporcion.ingresos.laborales.por.region.2018 <- EPH %>%
  filter(CH06 > 13) %>%
  filter(ESTADO == 1) %>%
  filter(Proporcion.ingresos.laborales > 0) %>%
  group_by(REGION) %>%
...
# General
Tablas_unidas_2018 <- data.frame(
  Año = 2018,
  Actividad = Actividad.2018[1, "Tasa.Actividad"],
  Empleo = Empleo.2018[1, "Tasa.Empleo"],
  Desempleo = Desempleo.2018[1, "Tasa.Desempleo"],
  Informalidad = Informalidad[1, "Tasa.Informalidad"],
  Subocupados = SubocupadoS.2018[1, "Tasa.Subocupados"]
)


write.csv(Tablas_unidas_2018, "data/eph/2018/General.csv")

# Por sexo
Tablas_unidas_sexo_2018 <- data.frame(
  Año = 2018,
  Sexo = Actividad.sexo.2018["Sexo"],
  Actividad = Actividad.sexo.2018["Tasa.Actividad"],
  Empleo = Empleo.sexo.2018["Tasa.Empleo"],
  Desempleo = Desempleo.sexo.2018["Tasa.Desempleo"],
  Informalidad = Informalidad.por.sexo["Tasa.Informalidad"],
  Subocupados = Subocupados.por.sexo.2018["Tasa.Subocupados"],
  Proporción_de_ingresos_laborales_sobre_totales = Proporcion.ingresos.laborales.por.sexo.2018["PromedioProporcion"]
)


write.csv(Tablas_unidas_sexo_2018, "data/eph/2018/Sexo.csv")


# Por región
Tablas_unidas_region_2018 <- data.frame(
  Año = 2018,
  Region = Actividad.Región.2018["REGION"],
  Actividad = Actividad.Región.2018["Tasa.Actividad"],
  Empleo = Empleo.región.2018["Tasa.Empleo"],
  Desempleo = Desempleo.región.2018["Tasa.Desempleo"],
  Informalidad = Informalidad.por.región["Tasa.Informalidad"],
  Subocupados = Subocupados.por.región.2018["Tasa.Subocupados"],
  Proporción_de_ingresos_laborales_sobre_totales = Proporcion.ingresos.laborales.por.region.2018["PromedioProporcion"]
)

write.csv(Tablas_unidas_region_2018, "data/eph/2018/Region.csv")
```